...
template<typename T> T tensorize(const mat & input, u_int dim, u_int dir)
{
    u_int degree = input.size() - 1;
...
template std::mat tensorize(const mat & input, u_int dim, u_int dir);
template boost::spmat tensorize(const mat & input, u_int dim, u_int dir);
