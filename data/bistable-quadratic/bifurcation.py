...
βmax, ε = 5.95, 0
betas = np.load("theta-1-white-noise-betas-up.npy")
ms = np.load("theta-1-white-noise-ms-up.npy")
...
ε = .1
betas = np.load("epsilon=1o10-m0=.75-betas.npy")
ms = np.load("epsilon=1o10-m0=.75-ms.npy")
condition = betas < βmax
betas, ms = np.extract(condition, betas), np.extract(condition, ms)
...
ε = .2
betas = np.load("epsilon=1o5-m0=.75-betas.npy")
ms = np.load("epsilon=1o5-m0=.75-ms.npy")
condition = betas < βmax
betas, ms = np.extract(condition, betas), np.extract(condition, ms)
ax.plot(betas, ms, '.-', color=cmap(ε), markersize=.5)
ax.plot(betas, -ms, '.-', color=cmap(ε), markersize=.5,
