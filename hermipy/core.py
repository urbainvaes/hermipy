...
def convert_to_cpp_vec(vec):
    cpp_vec = hm_cpp.double_vec()
    cpp_vec.extend(vec)
    return cpp_vec


def convert_to_cpp_mat(mat):
    cpp_mat = hm_cpp.double_mat()
    for vec in mat:
...
def to_csr(sp_matrix):
    if not isinstance(sp_matrix, hm_cpp.sparse_matrix):
        raise ValueError("Invalid argument!")
    rcv = np.array(hm_cpp.row_col_val(sp_matrix))
...
@log_stats()
def to_numpy(arg):
    if isinstance(arg, hm_cpp.double_vec):
        return np.array(arg)
    if isinstance(arg, hm_cpp.double_mat):
        return hm_cpp.to_numpy(arg)
    elif isinstance(arg, hm_cpp.boost_mat):
        return hm_cpp.to_numpy(arg)
    elif isinstance(arg, hm_cpp.double_cube):
        return np.array(arg)
    elif isinstance(arg, hm_cpp.sparse_matrix):
        return to_csr(arg)
...
def discretize(function, nodes, translation, dilation):
    ...
    return np.array(hm_cpp.discretize(function, nodes, translation, dilation))
...
def triple_products(degree):
    return np.array(hm_cpp.triple_products(degree))


def triple_products_fourier(degree):
    return np.array(hm_cpp.triple_products_fourier(degree))
...
def varfd(dim, degree, direction, var,
    ...
    if isinstance(var, np.ndarray):
        var = hm_cpp.to_boost_mat(var)
    elif isinstance(var, ss.csr_matrix):
        var = convert_to_cpp_sparse(var)
    result = log_stats()(hm_cpp.varfd)(dim, degree, direction, var,
...
def iterator_get_degree(dim, n_polys, index_set="triangle"):
    cpp_func = {
            "triangle": hm_cpp.triangle_get_degree,
            "cross": hm_cpp.cross_get_degree,
            "cross_nc": hm_cpp.cross_nc_get_degree,
            "cube": hm_cpp.cube_get_degree,
            "rectangle": hm_cpp.rectangle_get_degree,
            }
...
def iterator_size(dim, degree, index_set="triangle"):
    cpp_func = {
            "triangle": hm_cpp.triangle_size,
            "cross": hm_cpp.cross_size,
            "cross_nc": hm_cpp.cross_nc_size,
            "cube": hm_cpp.cube_size,
            "rectangle": hm_cpp.rectangle_size,
            }
    if index_set not in cpp_func:
        raise ValueError("Unknown index set")
...
def iterator_list_indices(dim, degree, index_set="triangle"):
    cpp_func = {
            "triangle": hm_cpp.triangle_list_indices,
            "cross": hm_cpp.cross_list_indices,
            "cross_nc": hm_cpp.cross_nc_list_indices,
            "cube": hm_cpp.cube_list_indices,
            "rectangle": hm_cpp.rectangle_list_indices,
            }
    if index_set not in cpp_func:
        raise ValueError("Unknown index set")
